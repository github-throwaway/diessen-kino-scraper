...
class FastMovieScraper:
    ...
    def _create_event_description(self, movie_details: Dict,
                                  booking_url: str) -> str:
        description_lines = [
            movie_details.get('description', ''),
            "",
            f"🕐 Dauer: {movie_details.get('duration', 120)} Minuten",
            f"🔞 FSK: {movie_details.get('fsk', '')}" if movie_details.get(
                'fsk') else None,
            f"🏛️ Saal: {movie_details.get('room', '')}" if movie_details.get(
                'room') else None,
            "",
            f"🎬 Trailer: {movie_details.get('trailer', '')}" if movie_details.get(
                'trailer') else None,
            "",
    ...
    async def get_movie_details(self, url: str) -> Dict:
        async with self.semaphore:
            try:
                async with self.session.get(
                        urljoin(self.base_url, url)) as response:
                    ...
                    if duration_elem:
                        ...
                        details['duration'] = int(
                            duration_match.group(1)) if duration_match else 120

                        fsk_match = re.search(r'FSK:\s*(\d+)', text)
                        details['fsk'] = fsk_match.group(
                            1) if fsk_match else ''

                    room_elem = soup.select_one(
                        '#sp-component div.cal-data-performance div:nth-child(1) span:nth-child(3)')
                    details[
                        'room'] = room_elem.text.strip() if room_elem else ''

                    trailer_elem = soup.find('iframe', src=lambda
                        s: s and 'youtube.com' in s)
                    details['trailer'] = trailer_elem[
                        'src'] if trailer_elem else ''
    ...
    async def scrape_movies(self) -> Optional[Calendar]:
        ...
        try:
            async with self.session.get(
                    f"{self.base_url}/im-kino-offcanvas/kinoprogramm/") as response:
                response.raise_for_status()
                html = await response.text()
                soup = BeautifulSoup(html, 'html.parser')
                ...
                cal.add('version', '2.0')
                cal.add('prodid',
                        '-//Kino Dießen Movie Schedule//diessen.de//')
                ...
                dates = [self._parse_date(th.text.strip().replace('\n', ''))
                         for th in table.find('thead').find_all('th')[1:]]
                movies = table.find('tbody').find_all('tr')

                # Concurrent movie details fetching with semaphore
                movie_details_tasks = [
                    self.get_movie_details(row.find('a')['href'])
                    for row in movies if row.find('a')]
                movie_details_results = await asyncio.gather(
                    *movie_details_tasks)
                    ...
                    for date, cell in zip(dates, screenings):
                        for time_link in cell.find_all('a'):
                            ...
                            description = self._create_event_description(
                                details, time_link['href'])
                            event.add('description', description)
                            events.append(event)

                    return events

                event_tasks = [process_movie(row, details)
                               for row, details in
                               zip(movies, movie_details_results)]
    ...
    def _parse_date(date_text: str) -> datetime:
        ...
        return datetime(datetime.now().year, int(match.group(2)),
    ...
    async def _create_html_page(self):
        try:
